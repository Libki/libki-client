...
PrintDialog::PrintDialog(SubmitPrintRequest &request, PrintInfoReply &info,
                         QWidget *parent)
    : QDialog(parent), request(request), info(info) {
  ...
  connect(copiesSpin, SIGNAL(valueChanged(int)), this, SLOT(updateTotals()));
  ...
  if (info.availableGratis) {
    if (info.gratisMethod == "pages") {
      availableGratisValue->setText(QString::number(info.availableGratis) +
                                    tr(" pages"));
      remainingGratisValue->setText(
          QString::number(info.remainingGratisBalance) + tr(" pages"));
    }
...
QString PrintDialog::money(double value) const {
  return QLocale().toCurrencyString(value, info.currency);
}
...
void PrintDialog::updateTotals() {
  ...
  if (info.gratisMethod == "pages") {
    if (totalPages <= info.availableGratis) {
      ...
      info.estimatedCost =
          info.costPerPage * (totalPages - info.availableGratis);
